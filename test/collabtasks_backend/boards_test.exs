defmodule CollabtasksBackend.BoardsTest do
  use CollabtasksBackend.DataCase

  alias CollabtasksBackend.Boards
  ...
  describe "boards" do
    ...
    test "create_board/1 with valid data creates a board" do
      valid_attrs = %{boardName: "some boardName"}

      assert {:ok, %Board{} = board} = Boards.create_board(valid_attrs)
      assert board.boardName == "some boardName"
    end
    ...
    test "update_board/2 with valid data updates the board" do
      board = board_fixture()
      update_attrs = %{boardName: "some updated boardName"}

      assert {:ok, %Board{} = board} = Boards.update_board(board, update_attrs)
      assert board.boardName == "some updated boardName"
    end
